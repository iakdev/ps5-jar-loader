        ...
        try {
            ...
            for (int i = 0; i < e_phnum; i++) {
                ...
                if (p_type != PT_LOAD || p_memsz == 0) {
                    Status.println("Skipping PHDR " + i + " (not PT_LOAD or memsz=0)");
                    continue;
                }
            ...
            if (base_addr.addr() != -1) {
                ...
                args[4] = 0;
                args[5] = 0;
                Status.println("Invoking entry point at " + (base_addr.inc(e_entry).addr()) + " with arg_addr=" + arg_addr.addr());
                libKernel.call(base_addr.inc(e_entry), args);
                Status.println("ELF execution completed");
            } else {
